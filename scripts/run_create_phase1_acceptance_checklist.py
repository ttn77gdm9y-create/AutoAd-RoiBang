...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Phase 1 create acceptance checklist.")
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError(
            "Phase 1 create acceptance checklist requires external_api_enabled=false and execution_enabled=false"
        )
        ...
        json.dumps(
            {
                ...
                "phase1_acceptance_status": result["phase1_acceptance_status"],
