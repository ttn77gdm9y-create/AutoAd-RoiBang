...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check Phase 2 勇者突进 manual preview config.")
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError("勇者突进配置检查要求 external_api_enabled=false 且 execution_enabled=false")
