...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Phase 1 local data sync.")
    parser.add_argument("--config", default="configs/runtime.example.json")
    parser.add_argument("--request", default="configs/data-sync.example.json")
    args = parser.parse_args()

    config = load_runtime_config(args.config)
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError("Phase 1 data sync requires external_api_enabled=false and execution_enabled=false")
