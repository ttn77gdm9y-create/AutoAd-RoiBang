...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build local source-material performance rollups from material_daily_metrics."
    )
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError("product source material rollup requires external_api_enabled=false and execution_enabled=false")
