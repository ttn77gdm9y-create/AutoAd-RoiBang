...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Phase 2 勇者突进 one-command preparation checks.")
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError("勇者突进一键准备检查要求 external_api_enabled=false 且 execution_enabled=false")
