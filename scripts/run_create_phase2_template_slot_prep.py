...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Phase 2 preparation template slot review pack.")
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError(
            "Phase 2 template slot preparation requires external_api_enabled=false and execution_enabled=false"
        )
