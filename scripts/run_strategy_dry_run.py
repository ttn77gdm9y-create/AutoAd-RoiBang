...
from roibang_v2.workflows.strategy_dry_run import run_strategy_dry_run_request
...
def run_from_args(argv: list[str] | None = None) -> int:
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError("Phase 1 strategy dry-run requires external_api_enabled=false and execution_enabled=false")
    ...
    strategy_preflight_path = (
        Path(args.strategy_preflight_artifact)
        if str(args.strategy_preflight_artifact or "").strip()
        else _latest_artifact(config.runs_dir, "strategy_preflight")
    )
    policy = load_json(args.policy).get("strategy_dry_run") or load_json(args.policy).get("strategy_plan") or {}
    result = run_strategy_dry_run_request(
        {
            "strategy_dry_run": {
                ...
                "strategy_preflight_artifact": load_json(strategy_preflight_path),
                "strategy_preflight_artifact_path": str(strategy_preflight_path),
                "policy": policy,
            }
        },
