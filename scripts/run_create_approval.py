...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Phase 1 create approval record.")
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError("Phase 1 create approval requires external_api_enabled=false and execution_enabled=false")
    ...
    policy = load_json(args.policy).get("create_approval") or load_json(args.policy).get("create_dry_run") or {}
    result = run_create_approval_request(
        {
            "create_approval": {
