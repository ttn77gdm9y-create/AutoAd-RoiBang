...
from roibang_v2.workflows.create_chain_index import WORKFLOWS, run_create_chain_index_request
...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Phase 1 create chain index.")
    parser.add_argument("--config", default="configs/runtime.example.json")
    args = parser.parse_args(argv)

    config = load_runtime_config(args.config)
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError("Phase 1 create chain index requires external_api_enabled=false and execution_enabled=false")
