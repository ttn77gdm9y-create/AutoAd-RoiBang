...
def run_from_args(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate readonly create-project suggestions from local data.")
    parser.add_argument("--config", default="configs/runtime.example.json")
    parser.add_argument("--strategy", action="append", default=[], help="策略 JSON，可重复；为空时读取策略目录下的 .local.json。")
    ...
    if config.external_api_enabled or config.execution_enabled:
        raise RuntimeError(
            "create_project_suggestions only reads local data and requires external_api_enabled=false and execution_enabled=false"
        )
