...
from roibang_v2.workflows.create_suggestion_strategy_review import build_create_suggestion_strategy_review
...
def _seed_project(tmp_path: Path, *, mode_key: str = "wx_pay_general_recent_scale") -> dict[str, Path]:
    ...
    if mode_key:
        _write_json(
            tmp_path / "configs" / "create-modes" / "demo-game" / f"{mode_key}.local.json",
            {
                "mode_key": mode_key,
    ...
    _write_json(
        strategy_path,
        {
            ...
            "enabled": True,
            "product_key": "demo-game",
            "mode_key": mode_key or "missing_mode",
            "target_date": "latest",
    ...
    _write_json(
        tmp_path / "configs" / "create-suggestion-strategies" / "example.json",
        {
            "strategy_id": "example-scale-capacity-v1",
            "strategy_version": "2026-05-31",
            "enabled": False,
            "product_key": "demo-game",
            "mode_key": "wx_pay_general_recent_scale",
            "account": {"require_allowed_account": True, "max_active_projects": 20, "cooldown_days_after_create": 2},
            "materials": {"window_key": "last_7d", "min_qualified_material_count": 5, "min_stat_cost": 200},
            "recommendation": {"project_count": 1, "units_per_project": 1, "daily_budget": 0},
        },
    )
...
def _load_script():
    script_path = Path("scripts/run_create_suggestion_strategy_review.py")
    spec = importlib.util.spec_from_file_location("run_create_suggestion_strategy_review", script_path)
...
def test_create_suggestion_strategy_review_estimates_hits_and_blocked_candidates(tmp_path: Path):
    ...
    assert result["workflow"] == "create_suggestion_strategy_review"
    assert result["execution_enabled"] is False
    assert result["external_api_calls"] == 0
    assert result["summary"]["strategy_count"] == 2
    assert result["summary"]["estimated_suggestion_count"] == 1
    assert result["summary"]["blocked_candidate_count"] == 1
    review = next(row for row in result["strategy_reviews"] if row["strategy_id"] == "recent-scale-capacity-v1")
    assert review["status"] == "warning"
    assert review["mode_exists"] is True
    assert review["template_exists"] is True
    assert review["candidate_account_count"] == 2
    assert review["qualified_material_count"] == 4
    assert review["estimated_suggestion_count"] == 1
    assert "已达到策略上限" in "；".join(review["top_blocking_reasons"].keys())


def test_create_suggestion_strategy_review_shows_recent_spend_scope_and_limit(tmp_path: Path):
    paths = _seed_project(tmp_path)
    _write_json(
        tmp_path / "configs" / "allowed-create-accounts.demo-game.local.json",
        {
    ...
    result = build_create_suggestion_strategy_review(
        ...
        mode_dir=tmp_path / "configs" / "create-modes",
        template_dir=tmp_path / "configs" / "create-templates",
        strategy_paths=[paths["strategy_path"]],
        product_key="demo-game",
    )

    review = result["strategy_reviews"][0]
    assert review["candidate_account_count"] == 2
    assert review["raw_suggestion_count"] == 2
    assert review["estimated_suggestion_count"] == 1
    assert review["max_suggestions_per_run"] == 1
    assert review["account_thresholds"]["candidate_scope"] == "allowed_accounts_with_recent_spend"
    assert review["account_thresholds"]["min_recent_stat_cost"] == 100
    assert any("收敛为 1 条建议" in warning for warning in review["warnings"])


def test_create_suggestion_strategy_review_lists_disabled_example_as_template(tmp_path: Path):
    paths = _seed_project(tmp_path)
    ...
    result = build_create_suggestion_strategy_review(
        ...
        mode_dir=tmp_path / "configs" / "create-modes",
        template_dir=tmp_path / "configs" / "create-templates",
        strategy_paths=[tmp_path / "configs" / "create-suggestion-strategies" / "example.json"],
        product_key="demo-game",
    )

    review = result["strategy_reviews"][0]
    assert review["status"] == "disabled"
    assert review["source_kind"] == "example"
    assert review["estimated_suggestion_count"] == 0
    assert any("示例策略模板" in item for item in review["repair_suggestions"])
    assert any("策略未启用" in item for item in review["warnings"])


def test_create_suggestion_strategy_review_blocks_missing_mode_config(tmp_path: Path):
    paths = _seed_project(tmp_path, mode_key="")
    ...
    assert review["mode_exists"] is False
    assert "创建模式配置不存在：missing_mode" in review["blocking_reasons"]
    assert any("missing_mode.local.json" in item for item in review["repair_suggestions"])


def test_create_suggestion_strategy_review_run_and_cli_are_readonly(tmp_path: Path):
    paths = _seed_project(tmp_path)
