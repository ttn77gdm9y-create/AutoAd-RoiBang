...
> 这是阶段 0 的历史收口记录。当前 `/suggestions` 已从“规则建议中心”演进为“投放建议工作台”，并已接入扩量机会、创建计划预览、项目管理建议和只读诊断分组；最新现状以 README 和架构说明为准。
...
- 阶段 0 当时的规则建议中心完成的是“巡检建议 -> 项目管理 JSON -> 项目管理页确认执行”的闭环。
- 阶段 0 当时的规则建议中心不生成创建项目建议；创建项目建议属于下一阶段。
...
- `/` 首页
- `/accounts` 产品账户库
- `/suggestions` 投放建议工作台（阶段 0 当时称“规则建议中心”）
- `/create-plans` 创建计划
