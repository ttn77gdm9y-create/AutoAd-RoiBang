...
CreateOpener = Callable[[str, dict[str, Any], dict[str, str], float], HttpResponse]
...
def build_create_http_transport(
    ...
    *,
    ...
    def transport(request: dict[str, Any]) -> dict[str, Any]:
        _operation, endpoint, payload = _validate_request(request)
        ...
        try:
            response = real_opener(url, payload, headers, timeout_seconds)
