...
- 规则建议中心：同步数据并重算建议、区分“可生成动作 JSON”和“只读观察/诊断”建议、把可执行建议生成项目管理 JSON，再跳转到项目管理页确认。
...
当前规则建议中心只处理项目管理类建议：删除项目、关闭项目、下调预算、下调出价，以及只读观察/诊断。真正的“创建项目建议”属于下一阶段，需要新增独立的数据证据、建议动作、预览和确认链路，不能混入当前项目管理建议闭环。
